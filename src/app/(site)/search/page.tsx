        ...
        {pages.map((page) => {
          ...
          return (
            <li key={page} style={{ ...liBase, margin: "0 3px" }}>
              <a
                href={`/search?${queryString(type, keyword, sort, page)}`}
                style={{
                  ...aBase,
        ...
        <div className="categorize shop-content">
            ...
            {TABS.map((tab, index) => {
              ...
              return (
                <li key={tab.key} style={{ display: "block" }}>
                  <a
                    href={`/search?${queryString(tab.key, keyword, sort, 1)}`}
          ...
          >
            ...
            >
              ...
              >
                <option value="consensus_desc">정확도순</option>
        ...
        <div className="view_box" style={{ padding: "50px 0" }}>
          {hits.length > 0 ? (
            ...
            >
              {hasKeyword
                ? "검색 결과가 없습니다."
                : "검색어를 입력하고 enter 또는 검색 버튼을 클릭해주세요."}
            </p>
          )}
        </div>
      </div>
